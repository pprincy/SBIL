...
@Component({
  selector: 'faimily-protection',
  templateUrl: 'faimily-protection.html'
})
export class FaimilyProtectionComponent {
  public steps;
  private amount:any;
  private min:any;
  private max:any;
  ...
  constructor(public utilitiesProvider : UtilitiesProvider, private numberPipe: DecimalPipe) {
    setTimeout(() => {
      ...
      this.amountComma = (this.rangeData.type=='r'?"₹":"") + val ? this.numberPipe.transform(val.toString().replaceAll(",","")) : "";
    }, 300);
  }
  ionViewDidEnter(){
