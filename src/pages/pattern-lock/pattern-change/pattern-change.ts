...
@IonicPage()
@Component({
  selector: 'page-pattern-change',
  templateUrl: 'pattern-change.html',
})
export class PatternChangePage {
  public lock: any;
  public setPattern = '';
  public confirmPattern = '';
  public patternSteps = 0;
  public oldPattern = '';
    ...
    this.lock = new patternLock('#patternHolder1', {
      onDraw: function (pattern) {
        if (pattern.length < 4) {
          ...
          if (that.patternSteps == 0) {
            that.oldPattern = pattern;
      ...
      this.restapiProvider.sendRestApiRequest(requestUpdateProfile, 'updateProfile')
        .subscribe((response) => {
          ...
          if (response.IsSuccess == true) {
            this.patternSteps++;
            this.myApp.updatePageUseCount("51");
            this.utilitiesProvider.googleAnalyticsTrackView('PatternChange');
