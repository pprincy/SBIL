...
@IonicPage()
@Component({
  selector: 'page-car-asset-allocation',
  templateUrl: 'car-asset-allocation.html',
})
export class CarAssetAllocationPage {
  ...
  ionViewDidLoad() {
    this.myApp.updatePageUseCount("30");
    this.utilitiesProvider.googleAnalyticsTrackView('Car Goal Allocation');
    this.scrollToTop();
    this.saveIncome();
  ...
  ionViewDidEnter() {
    this.utilitiesProvider.upshotScreenView('CarGoalSuggestion');
  }
  ...
  return this.restapiProvider.sendRestApiRequest(request, 'GetUserAssetLiabilities').subscribe((response) => {
    ...
    (error) => {
      ...
      takeScreenshot(){
        ...
        this.pageLoader = true;
        setTimeout(() => {
          this.utilitiesProvider.htmlToCanvas("capture-carasset").then(result => {
            setTimeout(() => {
              this.pageLoader = false;
            }, 1500);
          })
    ...
    saveFunction(type){
      if(type == "dont"){
          ...
          if(this.footerBtnType == 'cal'){
            ...
            this.navCtrl.setRoot('ArticlesPage',{source: 'Collections', header: this.utilitiesProvider.langJsonData['carGoal']['carGoal'], categoryID: 7});
          }
       }
      else{
        this.insertUserGoal();
    ...
    insertUserGoal(){
      ...
      let request = {
        ...
        "FVGoal":this.finalValue.FVCar,
        "FVDownPayment":this.finalValue.FVDownPayment,
