...
@IonicPage()
@Component({
  selector: 'page-retirement-asset-allocation',
  templateUrl: 'retirement-asset-allocation.html',
})
export class RetirementAssetAllocationPage {
  ...
  ionViewDidLoad() {
    this.myApp.updatePageUseCount("33");
    this.utilitiesProvider.googleAnalyticsTrackView('Retirement Goal Allocation');
  ...
  ionViewDidEnter() {
    this.utilitiesProvider.upshotScreenView('RetirementGoalSuggestion');
  }
...
replan(){
  this.navCtrl.setRoot('HomeGoalPage')
      ...
      takeScreenshot(){
        ...
        this.pageLoader = true;
        setTimeout(() => {
          this.utilitiesProvider.htmlToCanvas("capture-retirementallocation").then(result => {
            setTimeout(() => {
              this.pageLoader = false;
            }, 1500);
          })
    ...
    saveFunction(type){
      if(type == "dont"){
          ...
          if(this.footerBtnType == 'cal'){
            ...
            this.navCtrl.setRoot('ArticlesPage',{source: 'Collections', header: this.utilitiesProvider.langJsonData['retirementGoal']['retirementGoal'], categoryID: 5});
          }
       }
      else{
        this.insertUserGoal();
    ...
    insertUserGoal(){
      ...
      let request = {
        ...
        "GoalName":"Retirement",
        "GoalTypeID":"5",
        "CurrentAmount":this.finalValue.CurrentMonthlyExpenditure,
        "Period":this.finalValue.TimeToInvest,
        "LumpsumAvailable":this.finalValue.LumsumAvailable ,
        ...
        "EMIInterest":"",//
        "FVGoal": this.finalValue.AnnuityReceivablePerMonth,///
        "FVDownPayment":"",///
        "RequiredDownpayment":this.finalValue.ShortfallOfCorpus,//
    ...
    setUpshotEvent(action, saveGoal) {
      let payload = {
        ...
        "AnnualCorpus": this.finalValue.ShortfallOfCorpus,
