...
@IonicPage()
@Component({
  selector: 'page-education-asset-allocation',
  templateUrl: 'education-asset-allocation.html',
})
export class EducationAssetAllocationPage {
  ...
  constructor(public navCtrl: NavController, 
             ...
             for(let i = 0; i < this.table2.length; i++){
              ...
              if(this.table2[i].ProductCategory == "Debt") {
                this.debtPer = this.debtPer +  this.table2[i].AllocationPerc;
               this.debtList.push(this.table2[i]);
              }
              if(this.table2[i].ProductCategory == "Equity") {
                this.equityPer = this.equityPer +  this.table2[i].AllocationPerc;
  ...
  ionViewDidLoad() {
    this.myApp.updatePageUseCount("31");
    this.utilitiesProvider.googleAnalyticsTrackView('Education Goal Allocation');
    this.scrollToTop();
    this.saveIncome();
    ...
    this.utilitiesProvider.upshotScreenView('EducationGoalSuggestion');
  }
...
replan(){
  this.navCtrl.setRoot('HomeGoalPage')
      ...
      takeScreenshot(){
        ...
        this.pageLoader = true;
        setTimeout(() => {
          this.utilitiesProvider.htmlToCanvas("capture-educationasset").then(result => {
            setTimeout(() => {
              this.pageLoader = false;
            }, 1500);
          })
    ...
    saveFunction(type){
      if(type == "dont"){
          ...
          if(this.footerBtnType == 'cal'){
            ...
            this.navCtrl.setRoot('ArticlesPage',{source: 'Collections', header: this.utilitiesProvider.langJsonData['educationGoal']['educationGoal'], categoryID: 1});  
                    }
       }
      else{
        this.insertUserGoal();
    ...
    insertUserGoal(){
      ...
      let request = {
        ...
        "EMIInterest":"",//
        "FVGoal":this.finalValue.FVEducation,
        "FVDownPayment":"",//
        "RequiredDownpayment":this.finalValue.RequiredForEducation,//
    ...
    setUpshotEvent(action, saveGoal) {
      let payload = {
        ...
        "AnnualCorpus": this.finalValue.RequiredForEducation,
