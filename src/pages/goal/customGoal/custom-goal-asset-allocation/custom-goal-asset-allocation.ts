...
@IonicPage()
@Component({
  selector: 'page-custom-goal-asset-allocation',
  templateUrl: 'custom-goal-asset-allocation.html',
})
export class CustomGoalAssetAllocationPage  {
  ...
  constructor(public navCtrl: NavController, 
             ...
             this.goalId = this.paramsData.goalId;
             this.goalName = this.paramsData.goalName;
             this.goalImg = this.paramsData.imgFilled;
  ...
  ionViewDidLoad() {
    this.myApp.updatePageUseCount("34");
    this.utilitiesProvider.googleAnalyticsTrackView('Custom Goal Allocation');
    this.scrollToTop();
    this.saveIncome();
  ...
  ionViewDidEnter() {
   this.utilitiesProvider.upshotScreenView('CustomGoalSuggestion');
  }
  ...
  return this.restapiProvider.sendRestApiRequest(request, 'GetUserAssetLiabilities').subscribe((response) => {
    ...
    if (response.IsSuccess == true) {
      ...
      takeScreenshot(){
        ...
        this.pageLoader = true;
        setTimeout(() => {
          this.utilitiesProvider.htmlToCanvas("capture-customallocation").then(result => {
            setTimeout(() => {
              this.pageLoader = false;
            }, 1500);
          })
      ...
      disclaimerInfo(d , type){
       ...
       if(type == 'disclaimer'){
         ...
         this.disclaimerTitle = this.utilitiesProvider.commonLangMsg['disclaimer'];
         setTimeout(()=>{
    ...
    saveFunction(type){
      if(type == "dont"){
          ...
          if(this.footerBtnType == 'cal'){
            ...
            this.navCtrl.setRoot('ArticlesPage',{source: 'Collections', header: this.utilitiesProvider.langJsonData['customGoal']['custom'], categoryID: 8});
          }
       }
      else{
        this.insertUserGoal();
    ...
    insertUserGoal(){
      ...
      let request = {
        ...
        "GoalTypeID":1,
        "CurrentAmount":this.finalValue.AmountRequired,
        ...
        "FVDownPayment":"",//
        "RequiredDownpayment":this.finalValue.Diffrence,//
      ...
      return this.restapiProvider.sendRestApiRequest(request, 'InsertUserGoal').subscribe((response) => {
        this.pageLoader = false;  
          if (response.IsSuccess == true) {
            this.restapiProvider.presentToastTop(this.utilitiesProvider.commonLangMsg['goalSavedSuccessfully']);
            this.saveGoalPopup = ! this.saveGoalPopup;
            if(this.footerBtnType == 'cal'){
    ...
    setUpshotEvent(action, saveGoal) {
      let payload = {
        ...
        "AnnualCorpus": Number(this.finalValue.Diffrence) > 0 ? Number(this.finalValue.Diffrence) : 0,
        "SuggestedSaving": Number(this.finalValue.MonthlySavingRequired) > 0 ? Number(this.finalValue.MonthlySavingRequired) : 0,
